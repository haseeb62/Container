    ...
    with open(outputFile, "w") as outfile:
        with open(crossNamespaceFile) as f:
            for line in f:
                ...
                if temp["reader"]["mount namespace"] == hostNamespace:
                    for i in temp["writers"]:
                        if i["mount namespace"] == containerNamespace:
                            check = True
                        if i["mount namespace"] != containerNamespace:
                            remove.append(i)
                if check == True:
...
def generateBinary(input_file, output_file, query_file, inputLog_path, currentDirectory):
    ...
    with open(output_file, "w") as outfile:
        ...
        outfile.write("cd ~/{currentDir}\n".format(currentDir='/'.join(currentDirectory.split('/')[3:])))
        outfile.write("rm -r cross_graphs\n")
        outfile.write("mkdir cross_graphs\n")
        outfile.write("rm output.json\n")
        outfile.write("rm input*\n")
        outfile.write("rm cve.sh\n")
