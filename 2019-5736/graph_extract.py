    ...
    with open(input_file) as f:
        ...
        for line in f:
            ...
            if(temp["artifact"]["subtype"] == "file"):
                ...
                reader = {"pid": temp["reader"]["pid"],
                          "start time": temp["reader"]["start time"]}
                output["reader"] = reader
                writer_l = []
                for writer in temp["writers"]:
                    writer_l.append(
                        {"pid": writer["pid"], "start time": writer["start time"]})
...
def generateBinary(input_file, output_file, query_file, inputLog_path):
    ...
    with open(output_file, "w") as outfile:
        ...
        with open(input_file) as inputfile:
            ...
            for line in inputfile:
                ...
                with open(query_file + str(flow_count), "w") as qfile:
                    qfile.write('%artifact = "inode" == \'' +
                                flow["inode"] + '\'\n')
                    qfile.write(
                        '$artifact = $base.getVertex(%artifact)' + '\n')
                    qfile.write('%reader = "pid" == \'' + flow["reader"]["pid"] +
                                '\' and "start time" == \'' + flow["reader"]["start time"] + '\'\n')
                    qfile.write('$reader = $base.getVertex(%reader)' + '\n')
                    writer_count = 1
                    final_query = '$path = $base.getPath($reader, $artifact, 1)'
                    for writer in flow["writers"]:
                        qfile.write('%writer' + str(writer_count) + ' = "pid" == \'' +
                                    writer["pid"] + '\' and "start time" == \'' + writer["start time"] + '\'\n')
                        qfile.write('$writer' + str(writer_count) +
                                    ' = $base.getVertex(%writer' + str(writer_count) + ')\n')
                        final_query += ' + $base.getPath($artifact, $writer' + str(
                            writer_count) + ', 1)'
                        writer_count += 1
                    qfile.write(final_query + '\n')
                    qfile.write('export > /tmp/5736_' +
                                str(flow_count) + '.dot\n')
                    qfile.write('dump all $path\n')
                outfile.write(
                    "/home/vagrant/SPADE/bin/spade query < /home/vagrant/Container/2019-5736/inputs" + str(flow_count) + "\n")
                flow_count += 1
        # End loop
        outfile.write("./spade stop\n")
        outfile.write("truncate -s 0 ../cfg/spade.client.Control.config\n")
        outfile.write("./manage-postgres.sh clear\n")
        outfile.write("cd ~/Container/2019-5736\n")
        ...
        while count < flow_count:
            outfile.write("dot -Tsvg -o ~/Container/2019-5736/5736_" +
                          str(count) + ".svg 5736_" + str(count) + ".dot\n")
            with open("inputs" + str(count), "w") as inputs:
                inputs.write(
                    "load /home/vagrant/Container/2019-5736/" + query_file + str(count) + '\n')
                inputs.write("list\n")
                inputs.write("exit\n")
            count += 1
        outfile.write("rm 5736_*")
...
if __name__ == '__main__':
    ...
    getPrivilegedFlows(crossNamespaceFile, hostNamespace,
                       containerNamespace, privilegeFlowFile)
    extractFeatures(privilegeFlowFile, extractedFile)
    generateBinary(extractedFile, "cve.sh", "SPADE-queries",
                   "/home/vagrant/Container/2019-5736/5736_audit.log")
